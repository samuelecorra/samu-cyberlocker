...
![](../imgs/Pasted%20image%2020260315161544.png)
...
Soluzione proposta dalla prof su Ariel corso 24/25

![](../imgs/Pasted%20image%2020260315164855.png)

Si può ottenere un analogo risultato con CFSL estese (non so cosa sia la L nell'acronimo, finora non l'abbiamo citata o sbaglio? Preso e ricopiato quanto segue da Ariel corso 25/26) -- Secondo me è sbagliato o comunque piu impreciso del primo proposto sopra...

![](../imgs/photo_2026-03-15_16-43-12.jpg)

![](../imgs/photo_2026-03-15_16-43-15.jpg)
...
Qui non farei un unico mega-grafo, ma **5 piccoli grafi**, uno per ciascuna macchina.

![](../imgs/Pasted%20image%2020260315172115.png)
...
![](../imgs/photo_2026-03-15_17-43-42.jpg)

![](../imgs/photo_2026-03-15_17-43-45.jpg)
...
![](../imgs/Pasted%20image%2020260317014959.png)
...
- **Attività**: operazioni che richiedono un certo tempo
  ...
  ![](../imgs/Pasted%20image%2020260317015426.png)
...
![](../imgs/Pasted%20image%2020260317015031.png)
...
![](../imgs/Pasted%20image%2020260317015144.png)
...
![](../imgs/Pasted%20image%2020260317015540.png)

> [!example] Esempio: Lampada On/Off
> Se si è in `LampOn` (con `exit/printf("exiting")`) e si riceve il segnale
>  `off/printf("to off")` verso `LampOff` (con `entry/lamp.off()`), la sequenza è:
> 
...
![](../imgs/Pasted%20image%2020260317020041.png)
...
![](../imgs/Pasted%20image%2020260317020219.png)

> [!example] Esempio: Lampada con flashing
> Lo stato `LampFlashing` contiene due sotto-stati: `FlashOn` (con `entry/lamp.on()`) e `FlashOff` (con `entry/lamp.off()`), collegati da transizioni `after(1 sec)` che alternano l'accensione e lo spegnimento. Lo stato `LampFlashing` è raggiungibile mediante il segnale `<<signal>>flash/` ed è possibile uscirne con `<<signal>>on/` (verso `LampOn`). [p. 101]

Un altro esempio...

![](../imgs/Pasted%20image%2020260317020335.png)
...
![](../imgs/Pasted%20image%2020260317020401.png)
...
![](../imgs/Pasted%20image%2020260317020440.png)
...
![](../imgs/Pasted%20image%2020260317020503.png)
...
![](../imgs/Pasted%20image%2020260317015906.png)

Se adesso però valutassimo la questione con il branching dinamico condizionale...

![](../imgs/Pasted%20image%2020260317020141.png)
...
![](../imgs/Pasted%20image%2020260317020524.png)
...
![](../imgs/Pasted%20image%2020260317020544.png)
...
![](../imgs/Pasted%20image%2020260317161425.png)
...
![](../imgs/Pasted%20image%2020260317020638.png)

![](../imgs/Pasted%20image%2020260317194352.png)
...
![](../imgs/Pasted%20image%2020260317195435.png)

![](../imgs/Pasted%20image%2020260317020707.png)
...
![](../imgs/Pasted%20image%2020260317020829.png)
...
![](../imgs/Pasted%20image%2020260317020902.png)
...
![](../imgs/Pasted%20image%2020260317021104.png)
...
![](../imgs/photo_2026-03-17_20-46-23.jpg)

Riguardo al tuo dubbio sul **Sync State** (lo stato di sincronizzazione), capisco perfettamente il tuo punto di vista "da ingegnere", ma c'è una ragione tecnica per cui la tua professoressa non lo ha usato.

#### Perché non c'è un Sync State?

In UML, il **Sync State** (quello con le barre parallele di _fork_ e _join_) si usa solitamente all'interno di una **singola** macchina a stati che ha regioni ortogonali (che corrono in parallelo nello stesso oggetto).

Qui invece abbiamo **tre oggetti distinti**. La sincronizzazione avviene tramite lo scambio di **segnali (Signals)**. Ecco come il diagramma gestisce il coordinamento senza bisogno di nodi di sincronizzazione formali:

- **Sincronizzazione tramite Attesa:** Il Computer non prosegue "a caso". Entrando nello stato `ATTESA VERIFICA...`, si blocca implicitamente. È "sincronizzato" con il Telepass perché l'unico modo per uscire da lì è ricevere il segnale `send(OK)` o `send(NO)`.
    
- **Accoppiamento Debole:** Usare i segnali invece di un Sync State rende il sistema più realistico. Nella realtà, il Telepass e il Computer del casello sono macchine separate; non condividono un clock o una memoria comune, quindi "comunicano e attendono" invece di "fondersi" in un unico flusso parallelo.
    

---

#### Analisi della logica (giusto per confermare che è corretta)

Il flusso che hai disegnato segue una sequenza impeccabile:

1. **Trigger iniziale:** Il Computer riceve `<<signal>> auto` (sensore).
    
2. **Richiesta:** Il Computer invia i dati e si mette in "ascolto".
    
3. **Elaborazione:** Il Telepass esegue la `verify()`. Nota il **Decision Diamond** (il rombo giallo): è perfetto per modellare il bivio booleano `[check=OK]` vs `[check=NO]`.
    
4. **Risposta e Azione:** Il Computer riceve il responso. Se è OK, agisce sulla Sbarra con `open()`.
    
5. **Chiusura temporizzata:** Molto corretta l'integrazione del trigger temporale `after(1 min)` per inviare il `close()`.
    

#### Una piccola nota di "sicurezza" (visto che l'esame è quello)

L'unico dettaglio che un professore pignolo potrebbe chiederti è: _"Cosa succede se il Telepass non risponde mai?"_. Nel mondo del **Software Sicuro**, in quello stato di `ATTESA`, si potrebbe aggiungere un timeout (un altro `after(x sec)`) che porta a un errore o al display di "Servizio non disponibile", per evitare che il sistema rimanga appeso all'infinito.

Il diagramma è comunque una "Source of Truth" solida e formale.

![](../imgs/Pasted%20image%2020260317021135.png)
...
![](../imgs/Pasted%20image%2020260317021549.png)
...
![](../imgs/Pasted%20image%2020260317021610.png)
...
![](../imgs/Pasted%20image%2020260317021622.png)
...
![](../imgs/Pasted%20image%2020260317021632.png)

### 31.5 Diagnostica

Durante la fase di diagnostica, i tecnici controllano i **sensori** e tutte le **funzionalità** del dispositivo. Lo stato `InDiagnosi` coSntiene sotto-macchine per `Diagnostica1` e `Diagnostica2`, ciascuna con i propri step. [p. 130]

![](../imgs/Pasted%20image%2020260317021649.png)
...
![](../imgs/Pasted%20image%2020260317021704.png)

---

> **Fine del Blocco 6** — [pp. 111–132] ✅
> **Lezioni 3–4–5 completata.**