...
![](../imgs/Pasted%20image%2020260314175623.png)
...
La soluzione prevede quattro stati: `Off_pom`, `Off_cond`, `pompa`, `cond`. Le transizioni `ON` settano la variabile `temp` al valore di default; le transizioni `inv-est` commutano tra le due modalità anche da spento; `plus` e `minus` aggiornano `temp := temp ± 1`. [p. 59]

| Stato    | Evento  | Guardia | Update           | Nuovo stato |
| -------- | ------- | ------- | ---------------- | ----------- |
...
- **FSM I/O**: la soluzione usa 5 stati corrispondenti all'importo accumulato ($0, 10, 20, 30, 40$ centesimi), con transizioni esplicite per ogni combinazione.
...
| `ActiveNegative` | `three\_months` | saldo negativo mantenuto ininterrottamente per più di 3 mesi | nessuna oppure blocco amministrativo                            | `Blocked`          |
...
| Stato sorgente       | Evento         | Guardia                                        | Azione                                                                                     | Stato destinazione   |
| -------------------- | -------------- | ---------------------------------------------- | ------------------------------------------------------------------------------------------ | -------------------- |
| `Ready`              | `select(code)` | $available(code) = false$                      | $selected_code := code$; visualizza messaggio "bevanda non disponibile"                    | `Unavailable`        |
| `Ready`              | `select(code)` | $available(code) = true$                       | $selected_code := code$; $cost := price(code)$; visualizza $cost$; avvia timer di 1 minuto | `WaitingCard`        |
| `WaitingCard`        | `insert\_card` | $card_balance \ge cost$                        | $card_balance := card_balance - cost$; eroga bevanda                                       | `Dispense`           |
| `WaitingCard`        | `insert\_card` | $card_balance < cost$                          | visualizza messaggio "credito insufficiente"                                               | `InsufficientCredit` |
